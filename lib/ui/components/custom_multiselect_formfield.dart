      ...
      List<Widget> _buildSelectedOptions(state) {
        ...
        if (state.value != null) {
          state.value.forEach((item) {
            var existingItem = dataSource.singleWhere((itm) => jsonDecode(jsonEncode(itm))[valueField] == item, orElse: () => null);
            selectedOptions.add(Chip(
              label: Text(jsonDecode(jsonEncode(existingItem))[textField], overflow: TextOverflow.ellipsis),
            ));
          });
        }
      ...
      return InkWell(
        onTap: () async {
          ...
          dataSource.forEach((item) {
            items.add(MultiSelectDialogItem(jsonDecode(jsonEncode(item))[valueField], jsonDecode(jsonEncode(item))[textField]));
          });
